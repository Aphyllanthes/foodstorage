...
# Discussion
The FoodCoop Kornkammer obtains its products with the highest turnover rates from regional farmers (regional in this case defined as within a radius of 300 km). Nevertheless, it is not possible to speak of regional food, since countries in South- and Northamerica, Africa and Asia are found among the countries of origin.

The accuracy of information available on the origin of the products varies with the type of supplier and the product itself. The exact information about the origin is given above all by regional farmers. The reason is obvious, as these are mainly personal contacts. The exact information about the origin of a product from afar can almost only be provided by a cooperative, since wholesalers rarely provide this information. 
...
Since knowledge about the origin of our food is a matter of concern for the foodcoop, its members are already working on establishing further direct contacts with farmers/producers. Regarding the fact that the establishment of new connections to producers is an already runnig process, the data could be analysed in a future work regarding the time of order of the products. There might be a trend in increasing turnover rates of local products as well as increasing knowledge about the exact origin.

The calculated distances provide a good overview of the distances, but do not represent the distance the product was actually transported. Because we cannot expect to know the actual transport routes, but only make assumptions about it, the distance values are associated with uncertainties. Uncertainties increase with increasing distance, which we understand when we think of the transport routes that a cargo ship can take. Therefore, the actual approach routes were presented in this paper, but not used for calculations. 
Another uncertainty is that some products from suppliers even come from sub-suppliers. Tracing back the way of origin is very intransparent for some products (e.g. some products from the supplier "OBEG" come from "BYODO" - in this case, the condition that the information is available is fulfilled!). Due to the fact that the availability of such informations is rare, an implementation of the transport routes via two suppliers was not performed.

The calculation of the turnover in this work was done with values of different units. Most products turnover unit was kg, but few have the unit liters or pieces. A conversion of the units is still necessary, but was not possible in the time frame of this work, considering the error to be minor as only a few products are affected. It should therefore be taken into account that the results of this work are erroneous, especially regarding products like chocolate that comes in pieces of 100 gramms.
...
The purpose of the data visualisation in this work is to infrom consumers, but not to value the footprint of the emissions the transport caused. Therefore this work provides an overview on the origin of the products, which could also be used to calculate emissions in a future work, that likewise should be considered as overview and handled with respect to the inaccuracy of the data.
